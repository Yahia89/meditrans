...
export function usePermissions() {
  ...
  const can = (action: string): boolean => {
    ...
    if (userRole === 'patient') {
      const allowedActions = ['view_own_trips']
      return allowedActions.includes(action)
    }

    if (userRole === 'employee') {
      const allowedActions = ['view_dashboard', 'view_patients', 'view_drivers', 'view_employees', 'upload_files']
      return allowedActions.includes(action)
    }
  ...
  const isSuperAdmin = !!(profile?.is_super_admin || user?.email === import.meta.env.VITE_FOUNDER_EMAIL)
  ...
  return { can, userRole, isSuperAdmin, isOwner, isAdmin, isEmployee, isDriver, isPatient }
}
