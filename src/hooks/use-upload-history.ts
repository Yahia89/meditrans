  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: dbData, error: dbError } = await supabase
        ...
        .or(
          "purpose.is.null,purpose.not.in.(patient_document,driver_document,employee_document)"
        )
      ...
      storageData?.forEach((file) => {
        const isLinked = dbData?.some((dbRec) =>
          dbRec.file_path?.includes(file.name)
        );
        if (!isLinked) {
          records.push({
