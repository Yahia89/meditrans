...
export function TripDetails({ tripId, onEdit, onDeleteSuccess }: TripDetailsProps) {
    const { user } = useAuth();
    ...
    const updateStatusMutation = useMutation({
        mutationFn: async ({ status, requestedByDriver }: { status: TripStatus, requestedByDriver?: boolean }) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className={cn(
                                    ...
                                    trip.status === 'completed' ? "bg-slate-100 text-slate-600 border-slate-200" :
                                        trip.status === 'in_progress' ? "bg-blue-50 text-blue-700 border-blue-100" :
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                            <div className="space-y-6">
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Dropoff Location</p>
                        ...
                        {trip.notes && (
                            <div className="mt-12 p-6 bg-slate-50/50 rounded-2xl border border-slate-100">
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Special Instructions</span>
                                </div>
                                <p className="text-sm text-slate-600 leading-relaxed font-medium">{trip.notes}</p>
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-2xl bg-white border border-emerald-100 flex items-center justify-center shadow-sm">
                                    <HandPointing weight="duotone" className="w-6 h-6 text-emerald-600" />
                                </div>
                                <div>
                                    <h3 className="text-lg font-bold text-emerald-900">Driver Actions</h3>
                                    <p className="text-sm text-emerald-700/80">Manage the current state of this trip.</p>
                                </div>
                            </div>
                            <div className="flex gap-4 w-full md:w-auto">
                                ...
                                {canFinish && (
                                    <Button
                                        onClick={() => updateStatusMutation.mutate({ status: 'completed', requestedByDriver: false })} // Set to true to use foundation later
                                ...
                                {trip.status === 'completed' && (
                                    <div className="flex items-center gap-3 px-6 py-2 bg-white rounded-xl border border-emerald-100 text-emerald-700 font-bold shadow-sm">
                                        <CheckCircle weight="duotone" className="w-6 h-6 text-emerald-500" />
                                        Trip Completed
                                    </div>
                                )}
                            </div>
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                    <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-6">Patient Profile</h3>
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] uppercase font-bold tracking-wider text-slate-400 mt-0.5">Member since {trip.patient?.created_at ? new Date(trip.patient.created_at).getFullYear() : '2025'}</p>
                        </div>
                    </div>
                    <div className="space-y-4 pt-6 border-t border-slate-50">
                        ...
                        <div className="flex items-center gap-3 text-sm font-medium text-slate-600 truncate">
                            <Envelope weight="duotone" className="w-5 h-5 text-slate-400 shrink-0" />
                ...
                {/* Driver Profile */}
                <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                    <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-6">Assigned Driver</h3>
                    {trip.driver ? (
                        <>
                            <div className="flex items-center gap-4 mb-6">
                                <div className="w-12 h-12 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center">
                                    <Car weight="duotone" className="text-emerald-600 w-6 h-6" />
                                </div>
                                <div>
                                    <p className="font-bold text-slate-900">{trip.driver?.full_name}</p>
                                    <p className="text-[10px] uppercase font-bold tracking-wider text-slate-400 mt-0.5">Vehicle: {trip.driver?.vehicle_info || 'SEDAN'}</p>
                                </div>
                            </div>
                            <div className="space-y-4 pt-6 border-t border-slate-50">
                                <div className="flex items-center gap-3 text-sm font-medium text-slate-600">
                        ...
                        <div className="text-center py-8">
                            ...
                            <p className="text-sm font-bold text-slate-400">No driver assigned yet</p>
                            {canManage && (
                                <Button variant="link" className="text-xs text-blue-600 font-bold p-0 h-auto mt-2">
                                    Assign Now
                                </Button>
                            )}
                        </div>
            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent className="rounded-2xl border-slate-200">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogTitle className="text-xl font-bold text-slate-900">Delete Trip?</AlertDialogTitle>
                        <AlertDialogDescription className="text-slate-600">
                            This action cannot be undone. This will permanently delete the trip for
                            <span className="font-bold text-slate-900 ml-1">{trip.patient?.full_name}</span>.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
