    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between p-6 bg-white/70 backdrop-blur-md rounded-2xl border border-slate-200 gap-6 shadow-xl shadow-slate-200/40">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h3 className="font-black text-slate-800 text-xl tracking-tight truncate max-w-[200px] md:max-w-md">
              {file?.name}
            </h3>
              ...
              {sheets.map((sheet) => (
                ...
                >
                  <div className="flex items-center justify-between w-full min-w-[200px]">
                    ...
                    <span className="text-[10px] font-black text-slate-400 bg-slate-50 px-1.5 rounded">
                      {sheet.totalRows}
                    </span>
                  </div>
                </SelectItem>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                Automated normalization engine results
              </p>
            </div>
          </div>
        </div>
        <div className="overflow-x-auto max-h-[450px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-white/95 backdrop-blur-sm z-10">
              <tr>
                <th className="px-5 py-4 text-left font-black text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100 w-16">
                  #
                </th>
                {currentSheet.headers.map((header, i) => (
                  <th
                    key={i}
                    className="px-5 py-4 text-left font-black text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100 whitespace-nowrap min-w-[150px]"
                  >
                    {header}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-slate-50">
              {currentSheet.rows.slice(0, 20).map((row, rowIdx) => (
                ...
                >
                  <td className="px-5 py-4 text-slate-300 font-black text-xs group-hover:text-indigo-400 transition-colors">
                    {rowIdx + 1}
                  </td>
                  {currentSheet.headers.map((header, colIdx) => (
                    <td
                      key={colIdx}
                      className="px-5 py-4 text-slate-700 font-medium max-w-xs truncate group-hover:text-slate-900 transition-colors"
                    >
                      {row[header] ?? (
          ...
          {currentSheet.totalRows > 20 && (
            <div className="p-4 bg-slate-50/50 text-center border-t border-slate-100">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                Rendering first 20 records • {currentSheet.totalRows - 20} more
                rows hidden
              </p>
            </div>
          )}
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h4 className="font-black text-white text-xl tracking-tight">
                SmarterField™ Mapping
              </h4>
              <p className="text-indigo-200/60 text-[10px] font-bold uppercase tracking-widest mt-0.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {Object.entries(COLUMN_MAPPINGS[importSource]).map(
              ([targetField, sourceVariations]) => {
                // Use the same robust matching logic as in use-upload-flow.ts
                const matchedHeader = currentSheet.headers.find((h) => {
                  ...
                  return sourceVariations.some((variation) => {
                ...
                return (
                  ...
                  >
                    ...
                    {matchedHeader ? (
                      ...
                      <p className="text-white/20 text-sm font-bold italic tracking-wide">
                        Missing Data Source
                      </p>
                    )}
                  </div>
          ...
          <div className="mt-8 p-4 bg-white/5 rounded-2xl border border-white/5 backdrop-blur-md flex items-start gap-4 transition-all hover:bg-white/[0.08]">
            ...
            <p className="text-xs text-indigo-100/70 leading-relaxed font-medium">
              ...
              Any unmapped source columns will be automatically encrypted and
              stored as{" "}
              <code className="bg-indigo-500/30 text-white px-2 py-0.5 rounded font-mono text-[10px] border border-white/10">
                custom_fields
              </code>{" "}
              to prevent any data loss during the migration process.
            </p>
          </div>
        </div>
      </div>
