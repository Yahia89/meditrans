...
export function AcceptInvitePage() {
    ...
    const handleAccept = async () => {
        ...
        try {
            ...
            // 3. Update user profile default org if they don't have one
            ...
            // 4. Force refresh auth context to pick up new memberships
            await refreshAuth()
    ...
    if (error) {
        return (
            <div className="max-w-md mx-auto p-8 text-center space-y-6">
                ...
                <Button onClick={() => setPage('dashboard')} variant="outline" className="w-full">
                    Go to Login
                </Button>
        ...
        return (
            <div className="max-w-md mx-auto p-8 text-center space-y-6">
                ...
                <p className="text-slate-500">
                    You have successfully joined <strong>{invite.organizations.name}</strong> as an {invite.role}.
                </p>
                <Button onClick={() => setPage('dashboard')} className="w-full bg-[#3D5A3D] hover:bg-[#2E4A2E]">
                    Go to Dashboard
                </Button>
    ...
    return (
        <div className="max-w-md mx-auto p-8 space-y-8 bg-white rounded-2xl border border-slate-200 shadow-xl mt-12">
            <div className="text-center space-y-2">
                ...
                <p className="text-slate-500">
                    You've been invited to join <strong>{invite.organizations.name}</strong> as an <strong>{invite.role}</strong>.
                </p>
            </div>
            ...
            {!user ? (
                <div className="space-y-6">
                    <div className="flex p-1 bg-slate-100 rounded-lg">
                        <button
                            ...
                            className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${authMode === 'register' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}
                        >
                            Sign Up
                        </button>
                        <button
                            onClick={() => setAuthMode('login')}
                            className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${authMode === 'login' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}
                        >
                            Login
                        </button>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        >
                            {isAuthing ? <Loader2 className="h-4 w-4 animate-spin" /> : (authMode === 'login' ? 'Login & Accept' : 'Register & Accept')}
                        </Button>
                    </div>
                </div>
            ) : (
                ...
                <div className="space-y-4">
                    ...
                    <p className="text-[10px] text-center text-slate-400">
                        By clicking "Accept Invitation", you will be added to this organization.
                    </p>
                </div>
            )}
        </div>
